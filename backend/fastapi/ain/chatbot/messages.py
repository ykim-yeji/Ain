...
class IdealPersonMessage:
    ...
    def get_dialogs(self, thread_id, last_chat_id):
        if last_chat_id is None:
            thread_messages = self.client.beta.threads.messages.list(thread_id, limit=8)
        else:
            thread_messages = self.client.beta.threads.messages.list(thread_id, limit=8, after=last_chat_id)
