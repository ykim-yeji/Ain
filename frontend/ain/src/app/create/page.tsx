...
export default function CreatePage() {
  ...
  const submitInput = () => {
    if (genderInput !== null) {
      useCreateStore.setState((state) => ({
        mergeInput: faceInput + shapeInput + ectInput,
  ...
  return (
    <div className="mt-[65px] mb-[68px] w-full h-full flex flex-col">
      {/* 현재 인덱스 표시 */}
      <div className="w-full h-[15%] flex items-center justify-center">
        <div
          className={`w-3 h-3 mx-2 ${
            currentNum === 1 ? "bg-white" : "bg-[#D9D9D9]"
          } rounded-full shadow-md`}
        ></div>
        <div
          className={`w-3 h-3 mx-2 ${
            currentNum === 2 ? "bg-white" : "bg-[#D9D9D9]"
          } rounded-full shadow-md`}
        ></div>
        <div
          className={`w-3 h-3 mx-2 ${
            currentNum === 3 ? "bg-white" : "bg-[#D9D9D9]"
          } rounded-full shadow-md`}
        ></div>
        <div
          className={`w-3 h-3 mx-2 ${
            currentNum === 4 ? "bg-white" : "bg-[#D9D9D9]"
          } rounded-full shadow-md`}
        ></div>
        <div
          className={`w-3 h-3 mx-2 ${
            currentNum === 5 ? "bg-white" : "bg-[#D9D9D9]"
          } rounded-full shadow-md`}
        ></div>
      </div>
      ...
      <div className="w-full h-[85%] flex flex-row items-start justify-center">
        {currentNum !== 5 ? (
          <div className="w-[70%] h-[90%] bg-[#F0D5FA] rounded-xl shadow-lg  flex flex-col items-center">
            <div className="w-[80%] h-6 mt-[10%]">
              {currentNum != 1 && (
                <button onClick={(event) => setCurrentNum(currentNum - 1)}>
                  <img
                    src="./icon/angle_left_purple.png"
                    className="w-[10px]"
                  />
                </button>
              )}
            ...
            {currentNum == 1 ? (
              <div className="w-[80%] h-[60%] flex justify-center items-center bg-white rounded-xl px-4">
                <button
                  className={`flex flex-col items-center py-2 border-r-2 border-dashed rounded-md 
          ${genderInput === "MALE" ? "bg-[#C5EEFF]" : "hover:bg-[#C5EEFF]"}`}
                  onClick={(event) => {
                    setGenderInput("MALE"), setCurrentNum(2);
                  }}
                >
          ...
          ${genderInput === "FEMALE" ? "bg-[#FFD2F2]" : "hover:bg-[#FFD2F2]"}`}
                  onClick={(event) => {
                    setGenderInput("FEMALE"), setCurrentNum(2);
                  }}
                >
              ...
              <div className="w-[80%] h-[50%] flex justify-center items-center bg-white rounded-xl">
                {currentNum === 2 && (
                  <textarea
                    className="w-[90%] text-sm outline-none resize-none text-center"
                    rows={10}
                    value={faceInput}
            ...
            {currentNum !== 1 && (
              <div className="w-[80%] h-[25%] flex flex-col items-center justify-center">
                <button
                  className="w-[150px] h-[50px] bg-[#C776E3] rounded-2xl shadow-md text-center text-white text-lg hover:bg-[#AB42CF]"
                  onClick={(event) => setCurrentNum(currentNum + 1)}
                >
                  <p>입력 완료</p>
                </button>
                <button
                  className="text-xs mt-4 text-[#5F0F7A]"
                  onClick={(event) => setCurrentNum(currentNum + 1)}
                >
                  잘 모르겠어요..
                </button>
          ...
          <div className="w-[70%] h-full flex flex-col justify-start items-center">
            ...
            <div className="w-[90%] h-[220px] bg-[#F0D5FA] rounded-2xl flex flex-col items-center justify-evenly shadow-xl">
              ...
              <p className=" w-full text-lg text-center text-black">
                아인이 당신을 <br />
                기다리고 있어요!
              </p>
              <Link href="/loading" className="w-full flex justify-center">
                <button
                  className="w-[70%] h-14 mb-2 bg-[#AB42CF] rounded-xl text-center text-white shadow-md"
                  onClick={submitInput}
                >
                  만나러 가기
                </button>
