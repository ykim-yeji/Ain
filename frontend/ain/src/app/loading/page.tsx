...
export default function LoadingPage() {
  ...
  const fetchCharacterImage = async () => {
    ...
    try {
      await fetch(
        `${process.env.NEXT_PUBLIC_FAST_API_URL}/ideal-people/images`,
        {
          method: "POST",
          headers: {
  ...
  const fetchCharacterName = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/ideal-people/names?${queryParams}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="relative mt-[65px] mb-[68px] w-full h-full flex flex-col justify-center items-center">
      <img
        src="./gif/loading_circle.gif"
        className="w-[55%] rounded-full shadow-xl"
      />
      ...
      <img
        src="./background/loading_top.png"
        className="absolute top-0 right-0 w-1/3"
      />
      <img
        src="./background/loading_bottom.png"
